...
class JAWSCasemgrRecon(object):
    ...
    def execute_locally(self):
        ...
        logger.info(
            f"Read successful from Excel sheet. Available columns in Casemgr: {list(casemgr_data.columns)}"
        )
    ...
    def process_data(self, jaws_data, casemgr_data):
        ...
        # Check if the 'key' column exists before processing
        if "key" in case_df.columns:
            logger.info("'key' column exists, proceeding with splitting.")
            ...
            logger.error("'key' column not found in Case Manager data")
            raise KeyError("'key' column not found in Case Manager data")
